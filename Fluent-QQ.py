...
# 全局变量
server_ip = "http://pcfs.top:2/" # Fluent QQ Server 服务器地址 （尾部带斜杠）
...
icon = {'src': 'qq.ico','placement': 'appLogoOverride'}
minecraft_list = []
tabbar = None
isdarktheme = False
LM_Download_Way = {}
...
class SystemTrayIcon(QSystemTrayIcon):
    ...
    def __init__(self, parent=None):
        ...
        self.setIcon(QIcon('qq.ico'))  # 设置托盘图标
        self.parent = parent
...
class MainWindow(FluentWindow):
    def __init__(self):
        ...
        icon_path = os.path.join(os.getcwd(), 'qq.ico')
        ...
        if sys.platform == "win32":
            ...
            if error == 183:  # ERROR_ALREADY_EXISTS
                ...
                if not self.config.get('repeat_run', False):
                    ...
                    # 显示通知
                    notify(progress={
                        'title': f'Fluent QQ 已阻止了重复打开软件的操作',
                        'body': '为了防止 Fluent QQ 占满您的计算机，我们已阻止您重复打开 Fluent QQ\n如需重复打开，请到设置中勾选允许重复运行。',
                        'icon': os.path.join(os.getcwd(), 'qq.ico')
                    })
                    w = Dialog("Fluent QQ 已阻止了重复打开软件的操作", "为了防止 Fluent QQ 占满您的计算机，我们已阻止您重复打开 Fluent QQ\n如需重复打开，请到设置中勾选允许重复运行。")
                    if w.exec():
                        print('确认')
    ...
    def initNavigation(self):
        ...
        self.addSubInterface(self.homeInterface, QIcon("qq.ico"), "主页")
    ...
    def run_cmcl(self, version):
        ...
        if version not in minecraft_list:
            ...
            # 替换 CMCL 2.2.2 → Fluent QQ
            with open("run.ps1", "r+", encoding='utf-8') as f:
                content = f.read().replace('CMCL 2.2.2', 'Fluent QQ')
                f.seek(0)
                f.write(content)
                f.truncate()
            ...
            # 替换 CMCL → Fluent-QQ
            with open("run.ps1", "r+", encoding='utf-8') as f:
                content = f.read().replace('CMCL', 'Fluent-QQ')
                f.seek(0)
                f.write(content)
                f.truncate()
    ...
    class DownloadThread(QThread):
        ...
        def send_system_notification(self, title, message):
            try:
                if sys.platform == "win32":
                    toast(title, message, duration="short", icon={'src': 'qq.ico','placement': 'appLogoOverride'})  # 使用 win11toast 的 toast 方法
            except Exception as e:
                handle_exception(e)
    ...
    def handle_login(self, widget):
        ...
        if login_way_choose.currentText() == "离线登录":
            ...
            if not config.get('localmod', False):
                ...
                w = Dialog("您已启用本地模式", "Fluent QQ 在本地模式下无法进行微软登录，\n因为该操作需要互联网\n如果需要登录，请到设置界面关闭本地模式。或使用离线登录。")
                if w.exec():
                    print('确认')
                else:
                    print('取消')
    ...
    def update_log_clear_button_text(self, button):
        log_folder = os.path.join(os.getenv('APPDATA'), 'Fluent-QQ', 'log')
...
if not check_write_permission():
    w = Dialog("Fluent QQ 无法写入文件", "Fluent QQ 需要在安装文件夹写入文件，但是我们在多次尝试后仍无法正常写入文件\n这可能是由于安装文件夹是只读的。\n请考虑将百络谷启动器安装在非 Program Files , Program Files (x86) 等只读的文件夹\n由于没有写入权限，百络谷启动器将退出。")
    if w.exec():
        print('确认')
    else:
        print('取消')
